package btc.renaud.custombiome.command
...
import btc.renaud.custombiome.nms.NMSHandler
import btc.renaud.custombiome.util.WorldEditHandler
        ...
        greedyString("args") { argsStr ->
            executePlayerOrTarget { target ->
                ...
                try {
                    ...
                    // 4. Retrieve Bukkit Biome
                    // Since we just registered it, it should be available via Registry (Paper)
                    // Note: Registry might be cached, but NMS injection usually bypasses some caches or we might need to access via NMS
                    // Let's try standard Bukkit Registry first.
                     val biome = Bukkit.getRegistry(org.bukkit.block.Biome::class.java)?.get(randomKey)
                    ...
                    if (worldEditRegion != null) {
                        ...
                        // We need to adapt the apply logic to take a Region or iterate ourselves
                        // The CustomBiomeRegistry.applyBiome takes a location and radius.
                        // We should probably add a method to CustomBiomeRegistry or just do it here.
                        // For now, I'll allow applyBiome to support a loop or adding a new method is better.
                        // I'll implement a simple loop here for the selection.
                        ...
                        // Naive iteration (might be slow for huge regions, but acceptable for testing command)
                        // Better: Iterate chunks?
                        // For "setBiomeColor", precise block control is usually expected if selection is small, 
                        // but biomes are 4x4x4 (Quart) in modern versions. 
                        // world.setBiome writes to the quart.
